...
class ForgetPassWordVerifyViewController: UIViewController {
        ...
        func configureTextField() {
            ...
            @objc func handleRequest() {
                ...
                guard let verificationID = UserDefaults.standard.string(forKey: "forgetpass") else { return }
                ...
                Auth.auth().signInAndRetrieveData(with: credential) { (success, err) in
                    if err == nil {
                        print("Login Success")
                        let controller = ChangePassViewController()
    ...
    extension ForgetPassWordVerifyViewController : UITextFieldDelegate {
